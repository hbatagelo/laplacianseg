function output = LCdiff(im, num_iter, delta_t, kappa, option)
% Description:
%  Input(s):
%    im: gray scale image (M x N)
%    num_iter: number of iterations 
%    delta_t: integration constant (0 <= delta_t <= 1/7).
%    kappa: gradient modulus threshold which controls the conduction
%    options: conduction coefficient functions:
%       1 - c(x,y,t) = exp(-(nablaI/kappa).^2),
%       2 - c(x,y,t) = 1./(1 + (nablaI/kappa).^2),
% 
%  Outpu(s):
%     output: diffused image with the largest scale-space parameter
%
%   Example
%   -------------
...
%   kappa = 30;
%   option = 2;
%   output = LCdiff(s,num_iter,delta_t,kappa,option);
